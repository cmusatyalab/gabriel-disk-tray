#!/usr/bin/env python
#
# Application servers for using tensorflow models to do object detection.
#
...
class IkeaProxy(gabriel.proxy.CognitiveProcessThread):
    def __init__(self, image_queue, output_queue, task_server_addr, engine_id, log_flag=True):
        ...
        try:
            self.task_server_sock = zmqimagestream.zmqImageSender(connect_to=task_server_addr)
    ...
    def handle(self, header, data):
        ...
        # TODO: inefficient reencode the image
        encoded_img = cv2.imencode('.jpg', img)[1]
        self.task_server_sock.imsend("img", encoded_img, wait=False)
        objects_data = self.task_server_sock.zmq_socket.recv_json()

        # the object detection result format is, for each line: [x1, y1, x2, y2, confidence, cls_idx]
        # objects = np.array([objects_data])
...
if __name__ == "__main__":
    ...
    task_server_url = config.TF_TASK_SERVER_URL
    app_proxy = IkeaProxy(image_queue, result_queue, task_server_url, engine_id="Ikea")
    app_proxy.start()
